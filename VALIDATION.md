...
- Optimized Mac benchmarks measured all-book materialization at 2.335 ms versus 7.107 ms for JSON (52 books), and 22.811 ms versus 68.481 ms (500 repeated real stories). Onboarding's 5 KB file takes ~0.06 ms in cached runs. Measurements, limitations and repeatable commands are in diagnostics/library-loading/README.md.
- Fresh catalogue loading returns bundled values before saving. Post-launch sync owns the retryable, insert-if-absent import before any network work. Existing database/legacy migration paths remain intact, and a late bundled import cannot overwrite a downloaded revision. Onboarding readiness is independent of full-catalogue readiness; StoreKit access resolution is still required.
- Six new model tests cover real round trips, malformed data, isolated onboarding, delayed/failed import, persistence and stale-import protection. All 120 package tests pass; all app and test sources pass Swift 6 iOS SDK type-checking. Added an iOS RootViewModel gate test for onboarding while catalogue loading is suspended; simulator runtime execution remains outstanding. Preserved the user's PresentationTests/AppModelTests grouping and corrected SwiftPM exclusions to include nested model suites.
- Xcode's generation phase ran and both DAT resources copied successfully. A generic simulator build was attempted but storyboard/asset compilation failed because CoreSimulator had no available runtimes (iOS 26.2 platform unavailable). No app-launch performance claim is made from this environment. No test-process crash occurred in this pass.
